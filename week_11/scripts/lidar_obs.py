...
class Class_Name:  # 1단계: 클래스 이름 정의
    def __init__(self):  # 2단계: 클래스 초기화 및 초기 설정
        ...
        rospy.init_node("lidar_obstacle_node")  # ROS 1단계(필수): 노드 이름 정의
        ...
        self.obs_pub = rospy.Publisher("obstacle", String, queue_size=1)
        self.obs_dist_pub = rospy.Publisher("obstacle_dist", Int32, queue_size=1)
    ...
    def callback(self, msg):  # 3단계: 클래스 내의 콜백 함수 설정
        ...
        dist_list = []
        for index, value in enumerate(msg.ranges):  # LaserScan 메시지의 각도와 거리 정보를 반복하여 처리
            if 0 < msg.ranges[index] < 0.60 and abs(degrees[index]) < 30:
                # 거리가 0에서 0.60 사이이고 각도가 30도 이하인 경우:
        ...
        if len(object_degrees) > 5:  # 감지된 장애물의 각도 수가 5개 이상이면:
            print("Caution")  # 로봇을 정지 상태로 제어하기 위해 "STOP" 출력
            distance = Int32()
            distance = int(min(dist_list)*100)
            self.obs_pub.publish("Caution")
            self.obs_dist_pub.publish(int(min(dist_list)*100))
            print(int(min(dist_list)*100))
        else:
            print("GO")  # 그렇지 않으면 로봇을 이동 상태로 제어하기 위해 "GO" 출력
            self.obs_pub.publish("GO")
            self.obs_dist_pub.publish(100)
            print(100)
