...
class ImageProcessor:  # 1단계: 클래스 이름 정의
    def __init__(self):  # 2단계: 클래스 초기화 및 초기 설정
        ...
        self.L_R_Value = 0  # 좌측 또는 우측 뷰 선택을 나타내는 값을 초기화합니다. 0: 좌측, 1: 우측
    ...
    def run(self):
        ...
        if self.cam_flag:
            ...
            if self.L_R_Value == 0: 
                pos_lane = self.calc_left_distance(croped_img)
            else:
                pos_lane = self.calc_left_distance(croped_img) + cv_img.shape[1] // 2
            print(pos_lane)
            cv2.circle(cv_img, [pos_lane, self.y], 3, [0, 255, 0], -1)
